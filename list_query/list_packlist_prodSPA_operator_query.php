...
$packlist = $bdd->prepare('SELECT * FROM produits p, packlists pk, qualite q, pays py WHERE p.id_packlist = pk.id_packlist AND p.id_packlist = :id_packlist AND q.qualite = p.qualite AND py.id = pk.pays_id AND p.qualite = "Plast A"  ORDER BY p.qualite,p.nom_prod ASC');
